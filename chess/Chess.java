...
public class Chess extends Pane {
    ...
    /**
     * The given turn for the board. True = White, false = Black
     */
     ...
     */
    private KingLocation kingLoc;

    /**
     * When a pawn moves two squares forward this becomes that pawn. Used in en
     * passant.
     */
    private Piece doubleMovePawn;

    /**
     * a counter that goes up by 1 every move. If it hits 100 the game is a
     * draw. The counter gets reset when a pawn moves or when a piece is
     * captured.
     */
     ...
     */
    private HashMap<ArrayList<Integer>, Integer> threeRepetition;
     ...
     */
    private ArrayList<String> notationHistory;
     ...
     */
    private void initPiece(String url, int x, int y, boolean team, PieceType type) {
        Piece piece = new Piece(url, x, y, team, type);
     ...
     */
    public void makeMove(int oldX, int oldY, int newX, int newY, char pp) {
        if (!board[oldX][oldY].hasPiece()) {
            return;
        }
     ...
     */
    private void addMoveToHistory(int oldX, int oldY, int newX, int newY) {
        if (typeOfMove == MoveType.PAWN_PROMOTION) {
            history.add(oldX + " " + oldY + " " + newX + " " + newY + " "
                    + TYPE_TO_LETTER[board[newX][newY].getPiece().getType().type - 1]);
        } else {
            history.add(oldX + " " + oldY + " " + newX + " " + newY);
        }
        notationHistory.add(makeNotation(oldX, oldY, newX, newY));
    }

    /**
     * makeNotation takes in a starting location and ending location then turns
     * that into the standard algebraic notation used in chess.
     *
     * @param oldX starting x for the move
     * @param oldY starting y for the move
     * @param newX ending x for the move
     * @param newY ending y for the move
     * @return the formed algebraic move
     */
    private String makeNotation(int oldX, int oldY, int newX, int newY) {
        //To Do check && checkmate

        if (typeOfMove == MoveType.KING_CASTLE) {
            return "O-O";
        }
        if (typeOfMove == MoveType.QUEEN_CASTLE) {
            return "O-O-O";
        }

        //convert piece to standard chess location.
        Piece piece = board[newX][newY].getPiece();
        int convertedNumber = (newY + 1) * -1 + 9;
        String notation = TYPE_TO_LETTER[piece.getType().type - 1] + "";
        if (notation.equals(" ")) {
            notation = "";
        }
        ...
        for (int x = 0; x < WIDTH; x++) {
            for (int y = 0; y < HEIGHT; y++) {
                if (board[x][y].hasPiece()) {
                    ...
                    if (temp.getTeam() == piece.getTeam()
                            && temp.getType() == piece.getType()
                            && notationMove(x, y, newX, newY)) {
                        int[] loc = {x, y};
                        locations.add(loc);
                    }
                }
        ...
        for (int[] locs : locations) {
            if (locs[0] != oldX) {
                notation += (char) (oldX + 'a');
                break;
            }
        }
        for (int[] locs : locations) {
            if (locs[0] == oldX && locs[1] != oldY) {
                notation += oldY;
                break;
            }
        }

        //if attack then add x
        if (typeOfMove == MoveType.CAPTURE) {
            if (TYPE_TO_LETTER[piece.getType().type - 1] == ' ') {
                notation += (char) (oldX + 'a');
            }
            notation += "x";
        }
        ...
        //standard move
        notation += (char) (newX + 'a') + "" + convertedNumber;
        if (typeOfMove == MoveType.PAWN_PROMOTION) {
            notation = notation.substring(1);
            notation += "=" + TYPE_TO_LETTER[piece.getType().type - 1];
        }

        if (typeOfMove == MoveType.CHECK_MATE) {
            notation += "++";
        }
        if (typeOfMove == MoveType.CHECK) {
            notation += "+";
        }
     ...
     * draw conditions.
     */
    private void resetDrawConditions() {
     ...
     * @param pp char of the piece to promote to or ' ' if not piece specified
     */
    private void pawnPromotion(int x, int y, char pp) {
    ...
    /**
     * notationMove is used in the forming of the algebraic notation for the
     * current game. It attempts to move like pieces to the location to see if
     ...
     * @param newX ending x for the move
     * @param newY ending y for the move
     * @return whether or not the piece need to be disambiguated
     */
    private boolean notationMove(int oldX, int oldY, int newX, int newY) {
        Piece piece = board[newX][newY].getPiece();
        board[newX][newY].setPiece(null);
        boolean result = canMove(oldX, oldY, newX, newY);
        board[newX][newY].setPiece(piece);
        return result;
    }
     ...
     */
    private boolean pieceCanMove(int oldX, int oldY, int newX, int newY) {
        if (newX < 0 || newX > 7 || newY < 0 || newY > 7 || !board[oldX][oldY].hasPiece()) {
            return false;
        }
        return canMove(oldX, oldY, newX, newY) && !checkPieceBlock(oldX, oldY, newX, newY);
    }
     ...
     */
    private boolean inCheck(int kingX, int kingY) {
        changeTurn();
        for (int x = 0; x < WIDTH; x++) {
            for (int y = 0; y < HEIGHT; y++) {
    ...
    private boolean pawnMovementCheck(int oldX, int oldY, int newX, int newY) {
        ...
        if (newX == oldX) {
            ...
            if (Math.abs(newY - oldY) == 2) {
                if ((pieceTeam && !board[newX][newY + 1].hasPiece() && oldY == 6)
                        || (!pieceTeam && !board[newX][newY - 1].hasPiece() && oldY == 1)) {
                    return true;
                }
            }
        }
     ...
     */
    private boolean knightMovementCheck(int oldX, int oldY, int newX, int newY) {
     ...
     */
    private boolean horizontalMovementCheck(int oldX, int oldY, int newX, int newY) {
     ...
     */
    private boolean horizontalCheckForPieces(int oldX, int oldY, int newX, int newY) {
        if (oldX == newX) {
     ...
     */
    private boolean diagonalMovementCheck(int oldX, int oldY, int newX, int newY) {
     ...
     */
    private boolean diagonalCheckForPieces(int oldX, int oldY, int newX, int newY) {
